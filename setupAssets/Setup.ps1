...
$dataLakeAccountName = "stsql$suffix"
...
$cosmosDbAccountName = "cosmosdb-$suffix"
if ($cosmosDbAccountName.length -gt 43) {
    $cosmosDbAccountName = $cosmosDbAccountName.substring(0, 43)
}
$rgName = "rg-sqlfabric-$suffix"
...
-comosaccount $cosmosDbAccountName `
-Force
...
$destinationSasKey = New-AzStorageContainerSASToken -Container "sqldatabase" -Context $dataLakeContext -Permission rwdl
if (-not $destinationSasKey.StartsWith('?')) { $destinationSasKey = "?$destinationSasKey"}
$destinationUri = "https://$($dataLakeAccountName).blob.core.windows.net/sqldatabase$($destinationSasKey)"
$azCopy_Data_container = & $azCopyCommand copy "https://stfabcon.blob.core.windows.net/on-premisesdatacsvs" $destinationUri --recursive

$destinationSasKey = New-AzStorageContainerSASToken -Container "cosmosdb" -Context $dataLakeContext -Permission rwdl
if (-not $destinationSasKey.StartsWith('?')) { $destinationSasKey = "?$destinationSasKey"}
$destinationUri = "https://$($dataLakeAccountName).blob.core.windows.net/cosmosdb$($destinationSasKey)"
$azCopy_Data_container = & $azCopyCommand copy "https://stfabcon.blob.core.windows.net/cosmosdb" $destinationUri --recursive
...
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    ...
    foreach ($name in $cosmos) {
        ...
        foreach ($json in $document) {
            ...
            $res = New-CosmosDbDocument -Context $cosmosDbContext -CollectionId "Inventory_Data" -DocumentBody $body -PartitionKey $key 
        }
    } 

Write-Host  "-----------------Cosmos Data uploaded--------------"
